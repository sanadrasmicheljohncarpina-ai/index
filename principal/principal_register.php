...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== ($_SESSION['csrf_token'] ?? '')) {
        ...
        // Profile photo is optional, matching the standard faculty/staff registration.
        $photo_filename = null;
        ...
        if (empty($error)) {
            $result = ems_register_account($mysqli, 'principal', [
                ...
                'email'           => $email,
                'password'        => $password,
                // Principal registration intentionally does not ask for
                // Employee ID, Designation, or Assigned School Level.
                // Keep the existing principal scope behavior by defaulting to both.
                'education_level' => 'both',
            ]);

            if ($result['ok']) {
                // Attach the optional uploaded photo to the newly-created account.
                if ($photo_filename) {
                    $photoStmt = $mysqli->prepare("UPDATE users SET photo = ? WHERE username = ? AND role = 'principal' LIMIT 1");
                    if ($photoStmt) {
...
.reg-card{
    ...
    padding:36px 40px 30px;box-shadow:var(--shadow),0 0 0 1px rgba(217,154,43,.12);
}
.card-header{text-align:center;margin-bottom:22px;}
.logo-ring{width:68px;height:68px;border-radius:50%;border:2.5px solid var(--amber);box-shadow:0 0 20px rgba(217,154,43,.4);margin:0 auto 14px;display:block;object-fit:cover;}
...
.divider{height:1px;background:linear-gradient(90deg,transparent,rgba(217,154,43,.42),transparent);margin:0 0 20px;}
.section-title{font-family:'Rajdhani',sans-serif;color:var(--amber-hover);font-size:17px;letter-spacing:1px;text-transform:uppercase;display:flex;align-items:center;gap:8px;margin-bottom:4px;}
...
.photo-preview{width:78px;height:78px;border-radius:50%;background:var(--blue-inner);border:2px dashed rgba(217,154,43,.5);overflow:hidden;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:border-color .2s;flex-shrink:0;}
.photo-preview:hover{border-color:var(--amber);}
...
.btn-photo{display:inline-flex;align-items:center;gap:6px;background:rgba(217,154,43,.12);border:1px solid rgba(217,154,43,.35);color:var(--amber-hover);padding:7px 14px;border-radius:7px;font-size:12px;font-weight:600;cursor:pointer;margin-top:7px;}
...
.info-box i{color:#60a5fa;margin-top:1px;flex-shrink:0;}
.btn-register{width:100%;padding:13px;background:var(--amber);border:none;border-radius:var(--radius);color:#1a1204;font-size:15px;font-weight:700;font-family:'DM Sans',sans-serif;letter-spacing:.4px;cursor:pointer;margin-top:18px;transition:background .2s,transform .15s,box-shadow .2s;box-shadow:0 4px 16px rgba(217,154,43,.28);display:flex;align-items:center;justify-content:center;gap:8px;}
...
<div class="reg-card">
    ...
    <?php else: ?>
        ...
        <form method="POST" action="principal_register.php" enctype="multipart/form-data" id="regForm">
            ...
            <div class="info-box">
                ...
                <span>Your principal account will be submitted for <strong style="color:var(--light)">administrator approval</strong>. Your account details and access scope can be reviewed and managed later by an authorized administrator.</span>
            </div>
