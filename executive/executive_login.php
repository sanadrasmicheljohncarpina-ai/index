...
$stmt = $mysqli->prepare(
    "SELECT id, full_name, password_hash, role, designation, photo, account_status
     FROM users
     WHERE username = ? AND role = 'executive_assistant' AND is_active = 1 LIMIT 1"
);
...
<title>PBI — Executive Assistant Login</title>
<link href="https://fonts.googleapis.com/css2?family=Rajdhani:wght@600;700&family=DM+Sans:wght@400;500;600&display=swap" rel="stylesheet"/>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"/>
<style>
:root{--dark:#0A192F;--mid:#172A45;--violet:#7C3AED;--violet-h:#8B5CF6;--light:#E0E6F0;--muted:#A0B3C6;--radius:10px;--shadow:0 8px 32px rgba(0,0,0,0.45);}
...
.orb-1{width:400px;height:400px;background:radial-gradient(circle,rgba(124,58,237,.2) 0%,transparent 70%);top:-100px;right:-80px;animation:o1 14s ease-in-out infinite;}
...
.login-card{position:relative;z-index:10;background:rgba(23,42,69,.82);backdrop-filter:blur(18px);border:1px solid rgba(255,255,255,.09);border-radius:18px;padding:48px 44px 40px;width:100%;max-width:430px;box-shadow:var(--shadow),0 0 0 1px rgba(124,58,237,.15);animation:cardIn .7s cubic-bezier(.22,1,.36,1) both;}
@keyframes cardIn{from{opacity:0;transform:translateY(32px) scale(.97)}to{opacity:1;transform:none}}
.card-header{text-align:center;margin-bottom:28px;}
.logo-ring{width:74px;height:74px;border-radius:50%;display:block;object-fit:cover;border:2.5px solid var(--violet);box-shadow:0 0 22px rgba(124,58,237,.45);margin:0 auto 16px;}
...
.form-input:focus{border-color:var(--violet);box-shadow:0 0 0 3px rgba(124,58,237,.2);}
...
.secure-badge{display:inline-flex;align-items:center;gap:5px;font-size:11px;color:var(--muted);margin-top:14px;}
...
<div class="card-subtitle">Pandan Bay Institute Inc.</div>
        <div class="role-pill"><i class="fa-solid fa-briefcase"></i> Executive Assistant Access</div>
    </div>
    ...
    <form method="POST" action="executive_login.php">
    ...
    <div class="register-row">
        New to the system? <a href="executive_register.php">Register here</a>
    </div>
